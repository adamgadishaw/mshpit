...
export function LeftRail({ tab, setTab, session, unread = 0, notifUnread = 0, onLog, onFindVenues, onFanClubs, onNearby, onTopRated, onInbox, onActivity, onProfile, onEditProfile, onLogin }) {
  const NavItem = ({ item }) => {
    ...
    return (
      <Pressable style={[styles.navItem, on && styles.navItemOn]} onPress={() => setTab(item.key)}>
  ...
  return (
    <ScrollView style={styles.right} contentContainerStyle={styles.rightContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.card}>
        <View style={styles.cardHead}>
          ...
          <View style={styles.toggle}>
            <Pressable onPress={() => setArtistMode("top")} style={[styles.tgBtn, artistMode === "top" && styles.tgOn]}>
        ...
        {artists.map((a, i) => (
          <Pressable key={a.name} style={styles.aRow} onPress={() => onOpenArtist?.(a.name)}>
            {artistMode === "top" ? (
      ...
      <View style={styles.card}>
        ...
        {venues.map((v) => (
          <Pressable key={v.name} style={styles.aRow} onPress={() => onOpenVenue?.(v.name)}>
      ...
      <View style={styles.card}>
        ...
        {events.map((t) => (
          <Pressable key={t.id} style={styles.eRow} onPress={() => (onOpenEvent ? onOpenEvent(t) : onOpenArtist?.(t.artist))}>
...
const styles = StyleSheet.create({
  ...
  navItem: { flexDirection: "row", alignItems: "center", gap: 14, paddingVertical: 11, paddingHorizontal: 12, borderRadius: radius.md },
  navItemOn: { backgroundColor: colors.surface },
  navTxt: { color: colors.textDim, fontSize: 15, fontWeight: "600" },
  navTxtOn: { color: colors.amber, fontWeight: "800" },
  logBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: colors.amberStrong, borderRadius: radius.pill, paddingVertical: 12, marginTop: 12 },
  logTxt: { color: "#1A1206", fontSize: 14, fontWeight: "800" },
  ...
  link: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 9, paddingHorizontal: 12, borderRadius: radius.md },
  linkTxt: { color: colors.text, fontSize: 14, flex: 1 },
  ...
  card: { backgroundColor: colors.surface, borderRadius: radius.md, borderWidth: 1, borderColor: colors.lineSoft, padding: 14 },
  cardHead: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 },
  cardTitle: { color: colors.textFaint, fontSize: 11, letterSpacing: 1.5, fontWeight: "700" },
  seeAll: { color: colors.amber, fontSize: 12, fontWeight: "700" },
  toggle: { flexDirection: "row", backgroundColor: colors.bgElev, borderRadius: radius.pill, padding: 2, borderWidth: 1, borderColor: colors.line },
  tgBtn: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: radius.pill },
  tgOn: { backgroundColor: colors.amberStrong },
  tgTxt: { color: colors.textDim, fontSize: 11, fontWeight: "700" },
  tgTxtOn: { color: "#1A1206" },
  aRow: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 7 },
  ...
  aName: { color: colors.text, fontSize: 14, fontWeight: "700" },
  ...
  eRow: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 7 },
