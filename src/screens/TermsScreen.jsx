...
export default function TermsScreen({ onClose }) {
  return (
    <PolicyScreen
      ...
      sections={[
        { h: "Eligibility", p: "You must be at least 13 years old (or the minimum age required in your country) to use Pit, and legally able to enter this agreement. Up to two accounts may use one email address. Add a second account from Settings after confirming the email and the current account's password. Another email is needed for further accounts. Artist accounts are reviewed and verified before approval." },
        { h: "Shared email and account setup", p: "Each account has its own password. If a password matches both accounts using an email, sign-in asks which account to use. Changing a password affects only that account. Anyone with access to the shared inbox can receive recovery links for either account, so only share an email with someone you trust. Finish setup keeps your account; email verification is separate. Explicitly cancelling signup before Finish setup deletes the unfinished account and queues its uploads for removal. Closing the app, losing connection, or leaving setup unfinished does not delete the account or start an account-expiry timer. After finishing setup, use Delete account in Settings if you want to leave." },
