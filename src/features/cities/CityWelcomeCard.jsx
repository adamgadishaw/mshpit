...
export default function CityWelcomeCard({ city, onDismiss, onOpenCity, compact = false, hideActions = false }) {
  ...
  return <View style={styles.card}>
    <CityTicketTrim />
    {copy.welcomeBannerUrl ? <CityImage uri={copy.welcomeBannerUrl} contain={false} style={[styles.banner, compact && { aspectRatio: 4 }]} accessibilityLabel={copy.welcomeBannerAlt || cityName} priority="high" previewWidth={900} /> : null}
    <View style={[styles.body, compact && { padding: space(4), gap: space(2) }]}>
      <Text style={styles.kicker}>{cityText(copy, "welcomeKicker")}</Text>
      <Text style={[styles.title, compact && { fontSize: 23, lineHeight: 29 }]} accessibilityRole="header">{cityText(copy, "welcomeTitle", { city: cityName })}</Text>
      <Text style={styles.description}>{cityText(copy, "welcomeBody", { city: cityName })}</Text>
    </View>
    {(!hideActions || copy.welcomeBannerCredit) && <><CityTicketDivider />
    <View style={styles.actions}>
      {!hideActions && canOpen && onOpenCity ? <Button title={cityText(copy, "welcomeExplore")} onPress={() => onOpenCity(city)} /> : null}
      {!hideActions && <Button title={cityText(copy, "welcomeDismiss")} variant="secondary" onPress={onDismiss} />}
