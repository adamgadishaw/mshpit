...
Exact stored venue identities or unique normalized venue-name-and-city matches supply precise venue pins. A uniquely resolved, qualified city can supply a fallback pin labelled Approximate city location. The optional public concert address is a display label, not an exact geocoded point. Unknown and ambiguous locations remain in the list. Future plans, online reviews, and old Going/Interested records do not become attended concerts. The owner's already-recorded private Went entries remain in their own list, unpinned; they are never added to another viewer's response. When a review and private attendance entry identify the same venue and night, the review takes precedence. Venue-less posts can share a city pin but are never collapsed as duplicate nights; separate submitted addresses retain distinct list groups.
...
- A concert without coordinates remains listed with Location not mapped. Explicit city precision is labelled approximate. The client never infers coordinates from names or addresses. City-only rows use the city label instead of a fictional venue; address rows keep their submitted address, with the same approximate-city warning.
