...
test("composer people search is abortable, bounded, recoverable, and submitted", () => {
  ...
  assert.match(composer, /\{!isStatus && !isOnlineReview \? <AttachChip icon="you" label="People with you"/);
  ...
  assert.match(composer, /if \(isStatus \|\| isOnlineReview \|\| !showPeople \|\| query\.length < 2[\s\S]*?setPeopleError\(""\)/);
  assert.match(composer, /panels: \{ song: showSong, photos: showPhotos, people: !isStatus && !isOnlineReview && showPeople \}/);
  assert.doesNotMatch(composer, /showPlaylist|setShowPlaylist|label="Playlist"|Attach playlist|PlaylistAttachment/);
  assert.match(composer, /taggedPeople: isStatus \|\| isOnlineReview \? \[\] : taggedPeople/);
  assert.match(composer, /taggedPeople: isOnlineReview \? \[\] : taggedPeople/);
