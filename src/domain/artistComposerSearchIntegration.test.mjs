...
test("official provider titles remain distinct while clear tour names prefill the review", () => {
  ...
  assert.match(composer, /accessibilityLabel="Tour or special event name"/);
  assert.doesNotMatch(composer, /showTour/);
  ...
  assert.match(card, /\{reviewOnly \? "Open review" : "View this show"\}<\/Text>/);
