...
test("stale player routes are sanitized on restore, push, and replace", async () => {
  ...
  assert.match(app, /const commitGo = \(candidate\) => \{\s*let frame = navigationFrameForAccount\(prepareAvailableNavigationFrame\(candidate\), session\?\.id\);\s*if \(!frame\) return;/);
