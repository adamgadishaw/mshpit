...
test("incomplete setup is separate from email verification", () => {
  assert.equal(needsSignupOnboarding({ id: "new", emailVerified: true, onboardingVersion: 0 }), true);
  assert.equal(needsSignupOnboarding({ id: "new", emailVerified: false, onboardingVersion: 0 }), true);
...
test("legacy and signed-out sessions are not unexpectedly placed in onboarding", () => {
  ...
  assert.equal(needsSignupOnboarding({ id: "unproven", onboardingVersion: 0 }), true);
