...
const cleanCaption = (value, limit = 240) => String(value ?? "")
...
function artistMain(document) {
  ...
  const artistFacts = [
    ...
    artist.formed ? `<div><dt>Started</dt><dd>${esc(artist.formed)}</dd></div>` : "",
  ].filter(Boolean).join("");
...
function venueMain(document) {
  ...
  const heroPhotoUrl = publicMediaUrl(venue.heroPhoto?.url);
  ...
  const guide = venue.guide || { capacityLabel: null, seatingSummary: "Seating and capacity vary by event.", actions: [] };
  const guideActions = (guide.actions || []).flatMap((action) => {
    const href = publicHttpsUrl(action?.url);
    return href ? [`<a class="venue-guide-link" href="${esc(href)}" rel="nofollow noopener noreferrer"><strong>${esc(action.label)}</strong><span>${esc(action.description)}</span></a>`] : [];
  }).join("");
  const venueFacts = [
...
function directoryMain(document) {
  ...
  const venues = (document.venues || []).map((venue) => {
    ...
    const photoUrl = publicMediaUrl(venue.photo?.url || venue.image);
    const sourcePage = publicHttpsUrl(venue.photo?.sourcePage);
...
const STYLES = `
  ...
  *{box-sizing:border-box}html{background:#080807}body{margin:0;background:radial-gradient(circle at 76% 0,#2b1d0e 0,transparent 28rem),#080807;color:var(--ink);font:16px/1.55 ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;overflow:auto}
  ...
  .profile-hero h1{max-width:850px}.profile-hero .bio{max-width:760px;margin-top:1.6rem;color:#d8d0c5;font-size:1.1rem}.stats{display:flex;flex-wrap:wrap;gap:2.4rem;margin:2rem 0 0}.stats div{display:flex;flex-direction:column-reverse}.stats dt{color:var(--muted);font-size:.75rem;text-transform:uppercase;letter-spacing:.1em}.stats dd{margin:0;font:900 2rem/1 Georgia,serif}.stats small{font-size:.8rem;color:var(--muted)}.artist-guide-copy{max-width:760px;margin:1.25rem 0 0;color:#d8d0c5}.artist-facts{gap:.65rem}.artist-facts div{min-width:8.5rem;padding:.85rem 1rem;border:1px solid var(--line);border-radius:.9rem;background:var(--panel)}.artist-facts dd{font-size:1.2rem;line-height:1.15}.artist-next{max-width:760px;margin-top:1.25rem;padding:1rem 1.15rem;border-left:3px solid var(--gold);border-radius:0 .9rem .9rem 0;background:var(--panel)}.artist-next .eyebrow{margin-bottom:.35rem}.artist-next h2,.artist-next p{margin:.2rem 0}.artist-next h2{font-size:1.2rem}.artist-next p{color:var(--muted)}.event-list{list-style:none;padding:0;margin:0}.event-list li{display:grid;grid-template-columns:7rem 1fr auto;align-items:center;gap:1rem;padding:1.1rem 0;border-top:1px solid var(--line)}.event-list time{display:grid;gap:.2rem}.event-list time small,.archive-score{color:var(--muted);font-size:.75rem}.event-list h2,.event-list h3,.event-list p{margin:0}.event-list h2{font-size:1rem}.pill{border:1px solid var(--rose);border-radius:99px;padding:.25rem .55rem;color:var(--rose);font-size:.7rem}.event-facts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem;margin:2.5rem 0 0}.event-facts div{border-top:1px solid var(--line);padding-top:.8rem}.event-facts dt{color:var(--muted);font-size:.7rem;letter-spacing:.1em;text-transform:uppercase}.event-facts dd{margin:.25rem 0 0;font-weight:800}.ticket-action{display:flex;align-items:center;gap:1rem;margin-top:2rem}.ticket-action small{color:var(--muted)}.updates{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.8rem}.update p{margin:0}.update time{display:block;margin-top:1rem;color:var(--muted);font-size:.8rem}.member-hero{position:relative}.avatar{border:1px solid var(--line);border-radius:50%;object-fit:cover;margin-bottom:1.2rem}.avatar-fallback{display:grid;place-items:center;background:#241d12;color:var(--gold);font:900 3rem Georgia,serif}.comments ol{list-style:none;padding:0;margin:0}.comment{padding:1.2rem 0;border-top:1px solid var(--line)}.comment-reply{margin-left:clamp(1rem,5vw,4rem);border-left:2px solid var(--line);padding-left:1rem}.comment p{max-width:760px}.comment-meta{display:flex;justify-content:space-between;gap:1rem}.comment-meta time{color:var(--muted);font-size:.8rem}.empty-state{max-width:760px}.directory-grid{grid-template-columns:repeat(4,minmax(0,1fr))}
  .venue-hero{display:grid;grid-template-columns:minmax(0,1.08fr) minmax(0,.92fr);align-items:center;gap:clamp(1.5rem,5vw,4rem)}.venue-hero-photo{margin:0;overflow:hidden;border:1px solid var(--line);border-radius:1.25rem;background:#111}.venue-hero-photo img{width:100%;aspect-ratio:4/3;object-fit:cover}.venue-hero-photo figcaption,.directory-venue-photo figcaption{padding:.55rem .7rem;color:var(--muted);font-size:.7rem}.venue-hero-copy{min-width:0}.venue-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.55rem;margin:1.25rem 0 0}.venue-facts div{padding:.75rem;border:1px solid var(--line);border-radius:.8rem;background:var(--panel)}.venue-facts dt{color:var(--muted);font:.75rem ui-monospace,monospace;text-transform:uppercase;letter-spacing:.08em}.venue-facts dd{margin:.3rem 0 0;font-weight:800}.venue-guide-grid{display:grid;grid-template-columns:minmax(0,.8fr) minmax(0,1.2fr);gap:1rem}.venue-guide-grid>article,.venue-guide-actions{padding:1.1rem;border:1px solid var(--line);border-radius:1rem;background:var(--panel)}.venue-guide-grid h3{margin:.4rem 0;font-size:1.35rem}.venue-guide-actions{display:grid;gap:.65rem}.venue-guide-link{display:flex;padding:.75rem .85rem;justify-content:space-between;gap:1rem;border:1px solid var(--line);border-radius:.75rem;color:var(--ink);text-decoration:none}.venue-guide-link:hover{border-color:var(--gold)}.venue-guide-link span{color:var(--muted);font-size:.82rem;text-align:right}.venue-hero-fallback,.directory-venue-fallback{display:flex;min-height:20rem;padding:1.25rem;flex-direction:column;justify-content:flex-end;overflow:hidden;border:1px solid var(--line);border-radius:1.25rem;background:radial-gradient(circle at 72% 20%,#4a2717,transparent 45%),linear-gradient(145deg,#221c16,#090909);color:#fff}.venue-hero-fallback span,.directory-venue-fallback span{color:var(--gold);font:800 .68rem ui-monospace,monospace;letter-spacing:.16em}.venue-hero-fallback strong,.directory-venue-fallback strong{font:900 clamp(2rem,5vw,4rem)/.95 Georgia,serif}.venue-hero-fallback small{margin-top:.6rem;color:var(--muted)}.directory-venue-photo{margin:-1.25rem -1.25rem 1rem;overflow:hidden;border-radius:1rem 1rem 0 0;background:#0b0b0b}.directory-venue-photo img{width:100%;aspect-ratio:16/10;object-fit:cover}.venue-directory-card{overflow:hidden}.directory-venue-fallback{min-height:10rem;margin:-1.25rem -1.25rem 1rem;border:0;border-radius:1rem 1rem 0 0}.directory-venue-fallback strong{font-size:1.5rem}
