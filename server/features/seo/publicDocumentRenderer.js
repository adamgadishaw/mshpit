...
function venueMain(document) {
  ...
  return `<main id="main">
    ${breadcrumbs(document)}
    <section class="profile-hero venue-hero">${heroPhoto}<div class="venue-hero-copy"><p class="eyebrow">Live music venue</p><h1>${esc(venue.name)}</h1>${venue.place ? `<p class="hero-copy">${esc(venue.place)}</p>` : ""}${address}<p class="hero-copy venue-rating">${rating} · ${publicReviewCount}</p></div></section>
