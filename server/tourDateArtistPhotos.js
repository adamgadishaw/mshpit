...
import { deezerImageUrl } from "./features/artistPhotos/discoverPhoto.js";
...
// a Spotify link (see db.publicArtist), and the slideshow crops to fill. An
// artist whose page shows a Spotify photo lends its Discover-only Deezer image.
const MAX_KEYS = 500;
...
export function createTourDateArtistPhotoReader(database) {
  const statement = database.prepare(`SELECT a.norm,a.photo,
      ...
      CASE WHEN json_valid(a.data) THEN json_extract(a.data,'$.discoverPhoto.uri') END AS discover_uri,
      CASE WHEN json_valid(a.data) THEN json_extract(a.data,'$.discoverPhoto.credit') END AS discover_credit
    FROM artists a
    WHERE a.norm IN (SELECT value FROM json_each(?))
  ...
  return function withArtistPhotos(rows) {
    ...
    for (const row of statement.all(JSON.stringify(keys))) {
      const own = String(row.credit || "").trim() === "Deezer" ? deezerImageUrl(row.photo) : null;
      const uri = own || (row.discover_credit === "Deezer" ? deezerImageUrl(row.discover_uri) : null);
      if (uri) photos.set(row.norm, Object.freeze({ uri, credit: "Deezer" }));
    }
