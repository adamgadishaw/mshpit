...
test("privacy and terms mirror the dated in-app policies and expose support", () => {
  ...
  assert.match(privacy, /Spotify artist imagery/);
  assert.match(privacy, /server-side Spotify developer application/);
  assert.match(privacy, /sends only a known Spotify artist ID/);
  assert.doesNotMatch(privacy, /artist name or known Spotify artist ID/);
  assert.match(privacy, /Spotify-hosted artist image URL/);
  assert.match(privacy, /time Pit last checked the record/);
  assert.match(privacy, /does not connect a member(?:'|&#39;)s Spotify account/);
  assert.match(privacy, /request Spotify Personal Data or information about what a member plays/);
  assert.match(privacy, /does not set or use Spotify cookies/);
  assert.match(privacy, /Spotify Developer Terms/);
  assert.match(privacy, /Spotify Privacy Policy/);
  assert.doesNotMatch(privacy, /embedded YouTube player|preview audio|playback milestones/i);
  ...
  assert.match(terms, /Spotify artist data and images/);
  assert.match(terms, /warranties or representations on Spotify(?:'|&#39;)s behalf/);
  assert.match(terms, /implied warranties concerning the Spotify Platform/);
  assert.match(terms, /merchantability, fitness for a particular purpose, and non-infringement/);
  assert.match(terms, /modify or create derivative works/);
  assert.match(terms, /decompile, reverse-engineer, disassemble/);
  assert.match(terms, /Mshpit is solely responsible for Pit/);
  assert.match(terms, /Spotify is not responsible for Pit and has no liability/);
  assert.match(terms, /third-party beneficiary of these Terms and the Privacy policy/);
  assert.match(terms, /may directly enforce these Terms/);
  assert.match(terms, /Spotify Developer Terms/);
  assert.match(terms, /Spotify Privacy Policy/);
...
test("in-app legal copy matches the public provider disclosure and support route", async () => {
  ...
  assert.match(privacySource, /Spotify artist imagery/);
  assert.match(privacySource, /server-side Spotify developer application/);
  assert.match(privacySource, /sends only that ID to Spotify/);
  assert.match(privacySource, /request Spotify Personal Data or information about what a member plays/);
  assert.match(privacySource, /automatically makes a request to Spotify or its content-delivery service/);
  assert.match(privacySource, /Spotify Developer Terms/);
  assert.match(privacySource, /Spotify Privacy Policy/);
  ...
  assert.match(termsSource, /Spotify artist data and images/);
  assert.match(termsSource, /warranties or representations on Spotify's behalf/);
  assert.match(termsSource, /Spotify is an intended third-party beneficiary/);
  assert.match(termsSource, /Spotify Developer Terms/);
  assert.match(termsSource, /Spotify Privacy Policy/);
...
test("the HTTP server answers public documents before the SPA fallback", async () => {
  ...
  const publicAppRouter = source.indexOf("return serveSeoRoute(req, res, pathname, { search });", publicRoute);
...
test("public SEO responses are cacheable, canonical, and fail closed during projection outages", async () => {
  ...
  assert.match(source, /const responsePlan = hasOnlyPublicTrackingQuery\(search\) \? plan : \{ \.\.\.plan, indexable: false \}/);
  assert.match(source, /serveSeoRoute\(req, res, pathname, \{ search \}\)/,
    "tracking links retain clean canonical policy while functional/private queries remain noindex");
