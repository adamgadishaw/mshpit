...
test("a durable lease suppresses overlap, permits stale recovery, and seals sent days", () => {
  ...
  const first = claimSiteHealthDigest(db, { date: "2026-01-15", at });
  assert.equal(first.claimed, true);
  assert.equal(claimSiteHealthDigest(db, { date: "2026-01-15", at: at + 1 }).reason, "in-progress");
  ...
  assert.equal(claimSiteHealthDigest(db, { date: "2026-01-15", at: at + 2 * SITE_HEALTH_CLAIM_LEASE_MS }).reason, "sent");
});
...
test("the founder digest uses the fixed template, stable idempotency key, and sends once per day", async () => {
  ...
  assert.equal(calls[0][1].idempotencyKey, "site-health-2026-01-15");
