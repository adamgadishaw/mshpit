...
export function activeAccountSql(alias = "u") {
  ...
  return `${alias}.is_banned=0 AND ${alias}.dormant_at IS NULL AND (${alias}.suspended_until IS NULL OR ${alias}.suspended_until<=CAST(strftime('%s','now') AS INTEGER)*1000)`;
}

export function accountIsPublic(user, at = Date.now()) {
  // Dormancy is never a moderation ban and is cleared only by explicit auth.
  // The lifecycle writer and database guard exempt the durable Owner identity.
  return !!user && !user.is_banned && user.dormant_at == null
    && !(user.suspended_until && user.suspended_until > at);
}
