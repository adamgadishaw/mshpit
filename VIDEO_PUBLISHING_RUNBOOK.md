...
Universal admission (`universal-v1`, advertised as `universalAdmission` and `universalSourceTypes` in the signed health response) accepts every container in `VIDEO_SOURCE_MIME_TYPES` (`src/domain/mediaMime.mjs`): MP4, MOV, M4V, 3GP, WebM, Matroska, Ogg, AVI, MPEG-PS, MPEG-TS, WMV/ASF and FLV. The site sends a universal job, with no structural proof, only while the converter's fresh health advertises it. The converter forces the demuxer for the declared container, keeps the file-only protocol whitelist, selects the first decodable video stream (ignoring cover art, subtitles and data) and the first decodable audio stream, deinterlaces and squares pixels when needed, and always transcodes to the same H.264/AAC MP4 delivery, which is decoded strictly before publication. An unreadable or undecodable source is a signed `unsupported_media` rejection. AV1 is not decodable until libdav1d is added to the image. `node server/videoVerifierService.js --self-test` runs during the image build and converts real AVI, Matroska and interlaced MPEG clips.

Web-ready universal sources (H.264 with AAC or no audio in MP4/MOV/M4V/3GP, no rotation, square or unset pixels, at most 1920x1080 in either orientation, at most 60 fps, known length) are first repackaged without re-encoding (`-c copy` plus `h264_metadata=sample_aspect_ratio=1/1`). The copy must pass the strict delivery probe and full decode; if anything fails, the job converts in full, so the shortcut never refuses a clip. The self-test proves a portrait H.264/AAC MP4 takes this path and an anamorphic one does not.

Conversions are durable. The site records each one in `media_processing_jobs`; a restart makes interrupted conversions due, and the `/startup/video-processing` scheduler retries temporary failures (30 s, 1, 2, 5, 10, 20, 40 and 60 minutes; nine attempts) one clip at a time, only while the converter's health is fresh. A busy converter costs no attempt; `unsupported_media` (415) and a repeated conflict stop immediately. A clip can be posted while it converts: it is linked to the post, hidden until ready, and joins the post's `photos` in its original slot when the conversion commits. Only the author sees `convertingMedia` on the post. `POST /api/media/assets/:id/processing/retry` restarts a stopped conversion from the owner's original request.

Diagnosing a stuck clip: the converter's Render log has one `[video-verifier] start ...` line per job and then either `converted <type> <codecs> <size> <length> by remux|transcode in Ns (download, probe, convert, cover, upload times)` or `failed <type> at <step> after Ns (...): <code>: <FFmpeg's last lines>`. The site's log has `[media] clip conversion will retry automatically: attempt=N code=...` or `[media] clip conversion stopped after N attempt(s): code=... status=...`.

The strict path below still applies to a converter without universal admission. Current source-code bounds are at most 500 MiB and ten minutes, one supported progressive H.264 or HEVC video track, at most one AAC-LC mono/stereo audio track, and one verifier job at a time. The long edge is at most 4096 pixels and the short edge at most 2160, regardless of orientation. Source rates up to 240 fps share the unchanged total-frame and coded-pixel-work budget; delivery is capped at 60 fps. These ceilings are not a promise that a ten-minute 4K/240 fps clip fits the work budget. Corrupt, encrypted, unsupported Dolby Vision/hev1, and over-budget sources remain rejected. The server remains authoritative even when picker preflight passes. Verify the deployed source-support revision below before claiming the expanded formats are live.
