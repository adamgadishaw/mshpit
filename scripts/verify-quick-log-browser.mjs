...
async function scenario(browser, origin, width) {
  ...
  try {
    ...
    const details = page.getByRole("button", { name: "Detailed concert ratings", exact: true });
    const tourToggle = page.getByRole("button", { name: "Tour or special event details", exact: true });
    ...
    assert.equal(await page.getByLabel("Rating", { exact: true }).count(), 1, "Only one overall-experience rating appears initially.");
    assert.equal(await tour.count(), 0);
    assert.equal(await page.getByLabel("Public event address, optional", { exact: true }).count(), 0);
    ...
    await page.getByText("Date not remembered", { exact: true }).waitFor();
    await page.getByLabel("Rating", { exact: true }).press("End");
    assert.equal(await post.isEnabled(), true, "A city-only show with an unknown date and one rating must be publishable.");
    await details.click(); assert.equal(await page.getByLabel("Rating", { exact: true }).count(), 6);
    ...
    await details.click(); assert.equal(await page.getByLabel("Rating", { exact: true }).count(), 1);
    await tourToggle.click(); await tour.fill("A remembered tour"); await tourToggle.click();
    assert.equal(await tour.count(), 0); await tourToggle.click();
    assert.equal(await tour.inputValue(), "A remembered tour"); await tourToggle.click();
    ...
    await page.getByRole("button", { name: "Choose concert date", exact: true }).scrollIntoViewIfNeeded();
